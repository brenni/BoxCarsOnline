...
minimapcamera.width = 30*minimapscale+"px";
...
cw_Car.prototype.__constructor = function(car_def) {
  ...
  this.is_enemy = false;
    this.colour = car_def.colour;
    this.userId = car_def.userId;
...
cw_Car.prototype.kill = function() {
  ...
  cw_carScores.push({ car_def:this.car_def, v:score, s: avgspeed, x:position, y:this.maxPositiony, y2:this.minPositiony });
    //console.log("Car is: "+this.car_def.userId);
...
function cw_pushTop(){
  ...
  xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    ...
    //cw_carScores[0].car_def.colour = colour;
    
  // send the collected data as JSON
  xhr.send(JSON.stringify(cw_carScores[0]));
        }
     //console.log("Winning car #"+cw_carScores[0].car_def.index+" driven by: "+cw_carScores[0].car_def.userId);
}
...
function cw_materializeGeneration() {
  ...
  var JSONObj = '';
        ...
        jsonhttp.onreadystatechange = function() {
        	...
        	for(var i = 0; i < 10; i++){
            		cw_carGeneration.push(JSONObj[i]);
       		}
        	generationSize = cw_carGeneration.length;
                
        	 for(var k = 0; k < generationSize; k++) {
                
           	        cw_carGeneration[k].index = k;
   		               cw_carArray.push(new cw_Car(cw_carGeneration[k]));
   		 	if(k >= 19){
       				 cw_carArray[k].is_enemy = true;
    			}
  		}
                
		//generationSize = cw_carArray.length;
...
function cw_nextGeneration() {
  ...
  var newborn;
  cw_getChampions();
  //cw_topScores.push({i:cw_carScores[0].car_def.userId,v:cw_carScores[0].v,x:cw_carScores[0].x,y:cw_carScores[0].y,y2:cw_carScores[0].y2, c:cw_carScores[0].car_def.colour});
  cw_pushTop();    
    cw_pullTop();
  plot_graphs();
  for(var k = 0; k < gen_champions; k++) {
      //console.log("Best car is: "+JSON.stringify(cw_carScores[0]));
    cw_carScores[k].car_def.is_elite = true;
    cw_carScores[k].car_def.index = k;
    newGeneration.push(cw_carScores[k].car_def);
  }
  for(k = gen_champions; k < (generationSize  - 10); k++) {
...
function cw_makeChild(car_def1, car_def2) {
  ...
  newCarDef.chassis_density = parents[curparent].chassis_density;
    newCarDef.userId = username;
    newCarDef.colour = colour;
  return newCarDef;
}
...
function restoreProgress() {
  ...
  floorseed = "db3cbc01da600701b9fe4a497fe328e71fa7022f";
